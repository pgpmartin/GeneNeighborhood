...
```{r randUpstreamDist, results = "hide", warning=FALSE}
...
```{r randDist, results = "hide", warning=FALSE}
...
```{r randDist CI values kable, echo = FALSE}
randDist$stats  %>% 
    dplyr::filter(GeneGroup == "GeneList") %>%
    dplyr::select(GeneGroup:n, 
                  Median_lowerCI, Median, Median_upperCI, 
                  Mean_lowerCI, Mean, Mean_upperCI) %>%
  knitr::kable(digits = c(rep(1,4), rep(2,6)), 
               format = "markdown",
